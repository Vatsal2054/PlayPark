...
export default function DashboardContent() {
  ...
  const stats = [
    ...
    },
    {
      ...
      value: gameNights.reduce(
        (total, gn) => total + (gn.confirmedPlayers?.length || 0),
        0
      ),
      icon: Users,
    ...
    <div className="space-y-8">
      ...
      >
        ...
        <p className="text-gray-600 max-w-2xl mx-auto">
          Organize amazing game nights, connect with friends, and create
        ...
        {stats.map((stat, index) => {
          ...
          return (
            <motion.div
              ...
              className="group"
            >
              <Card className="border-0 shadow-sm hover:shadow-lg transition-all duration-300 bg-white/80 backdrop-blur-sm">
            ...
            return (
              <Link key={action.name} to={action.path}>
                <motion.div
                  ...
                  whileTap={{ scale: 0.98 }}
                  className="p-4 rounded-lg bg-gradient-to-r from-purple-50 to-indigo-50 border border-purple-100 hover:border-purple-200 transition-all duration-300 group"
                >
                  <div className="flex items-center gap-3">
                    <IconComponent className="h-5 w-5 text-purple-600 group-hover:scale-110 transition-transform" />
